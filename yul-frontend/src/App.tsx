...
function App() {
  ...
  useEffect(() => {
    if (corpInfo && corpInfo.proposalCount > 0) {
      loadProposals()
    }
        ...
        <main className="flex-1 min-w-0">
          ...
          {!account ? (
            <div className="flex flex-col items-center justify-center h-96 gap-6">
              ...
              <h2 className="text-2xl font-bold text-gray-300">Connect Your Wallet</h2>
              <p className="text-gray-500 text-center max-w-md">
                Connect your Ethereum wallet to interact with the YUL Corporation protocol.
              </p>
              <button
                onClick={connectWallet}
            ...
            <div className="flex flex-col items-center justify-center h-96 gap-6">
              <div className="text-6xl">📋</div>
              <h2 className="text-2xl font-bold text-gray-300">Set Contract Addresses</h2>
              <p className="text-gray-500 text-center max-w-md">
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold">
        {corpInfo ? corpInfo.name : 'Corporation Dashboard'}
      </h1>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-gray-200">Your Position</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gray-800/50 rounded-lg p-4">
            <div className="text-sm text-gray-400">Share Balance</div>
            <div className="text-2xl font-bold text-violet-300">{Number(shareBalance).toLocaleString()}</div>
            <div className="text-xs text-gray-500">YUL</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-4">
            <div className="text-sm text-gray-400">Voting Power</div>
            <div className="text-2xl font-bold text-indigo-300">{Number(votingPower).toLocaleString()}</div>
            <div className="text-xs text-gray-500">votes</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-4">
            <div className="text-sm text-gray-400">Role</div>
            <div className="text-2xl font-bold text-emerald-300">{isFounder ? 'Founder' : 'Shareholder'}</div>
      ...
      {proposals.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <h2 className="text-lg font-semibold mb-4 text-gray-200">Recent Proposals</h2>
          <div className="space-y-3">
            {proposals.slice(0, 5).map((p) => (
              <div key={p.id} className="flex items-center justify-between bg-gray-800/50 rounded-lg p-3">
                <div className="flex items-center gap-3">
                  <span className="text-gray-500 font-mono text-sm">#{p.id}</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <div className="text-sm text-gray-400 mb-1">Your Share Balance</div>
          <div className="text-3xl font-bold text-violet-300">{Number(shareBalance).toLocaleString()}</div>
          <div className="text-sm text-gray-500">YUL tokens</div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <div className="text-sm text-gray-400 mb-1">Voting Power</div>
          <div className="text-3xl font-bold text-indigo-300">{Number(votingPower).toLocaleString()}</div>
          <div className="text-sm text-gray-500">delegated votes</div>
        </div>
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-gray-200">Delegate Votes</h2>
        <p className="text-sm text-gray-400 mb-4">
          Delegate your voting power to yourself or another address. You must delegate to activate your voting power.
        </p>
        <div className="flex gap-3">
          <input
            type="text"
            placeholder="Delegate address (or leave empty to self-delegate)"
            value={delegateTo}
            onChange={(e) => setDelegateTo(e.target.value)}
            className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-sm text-gray-300 focus:border-violet-500 focus:outline-none"
          />
          <button
            onClick={() => delegateVotes(delegateTo || account)}
            disabled={loading}
            className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-gray-700 text-white px-6 py-2 rounded-lg text-sm font-medium transition-colors"
          >
      ...
      {isFounder && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <h2 className="text-lg font-semibold mb-4 text-gray-200">Issue Shares</h2>
          <p className="text-sm text-gray-400 mb-4">
            As the founder, you can issue new shares to shareholders.
          </p>
          <div className="space-y-3">
            <input
              type="text"
              ...
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-sm text-gray-300 focus:border-violet-500 focus:outline-none"
            />
            <div className="flex gap-3">
              <input
                type="text"
            ...
            <button
              onClick={() => issueShares(issueTo, issueAmount, issueReason)}
              disabled={loading || !issueTo || !issueAmount}
              className="bg-violet-600 hover:bg-violet-500 disabled:bg-gray-700 text-white px-6 py-2 rounded-lg text-sm font-medium transition-colors"
            >
...
function GovernancePage({ proposals, loading, createProposal, castVote, executeProposal }: {
  ...
  return (
    <div className="space-y-6">
      ...
      {showCreate && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <h2 className="text-lg font-semibold mb-4 text-gray-200">Create Proposal</h2>
          <div className="space-y-3">
            <div>
              <label className="text-sm text-gray-400 mb-1 block">Proposal Type</label>
              <select
                value={propType}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-sm text-gray-300 focus:border-violet-500 focus:outline-none h-24 resize-none"
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                <label className="text-sm text-gray-400 mb-1 block">Target Address</label>
                <input
                  type="text"
                  value={propTarget}
              ...
              <div>
                <label className="text-sm text-gray-400 mb-1 block">ETH Value</label>
                <input
                  type="text"
                  value={propValue}
              ...
              <div>
                <label className="text-sm text-gray-400 mb-1 block">Calldata</label>
                <input
                  type="text"
                  value={propCalldata}
            ...
            <button
              onClick={() => {
                createProposal(propType, propDesc, propTarget, propValue, propCalldata)
                setShowCreate(false)
              }}
            ...
            <div key={p.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <h3 className="text-gray-200 font-medium">{p.description}</h3>
              ...
              {p.state === 1 && (
                <div className="flex gap-2">
                  <button onClick={() => castVote(p.id, 1)} disabled={loading}
                    className="bg-green-600/20 hover:bg-green-600/30 text-green-400 px-4 py-1.5 rounded-lg text-xs font-medium transition-colors border border-green-600/30">
                    Vote For
                  </button>
                  <button onClick={() => castVote(p.id, 0)} disabled={loading}
                    className="bg-red-600/20 hover:bg-red-600/30 text-red-400 px-4 py-1.5 rounded-lg text-xs font-medium transition-colors border border-red-600/30">
                    Vote Against
                  </button>
                  <button onClick={() => castVote(p.id, 2)} disabled={loading}
                    className="bg-gray-600/20 hover:bg-gray-600/30 text-gray-400 px-4 py-1.5 rounded-lg text-xs font-medium transition-colors border border-gray-600/30">
                    Abstain
                  </button>
                </div>
              )}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          ...
          <div className="text-3xl font-bold text-emerald-300">{corpInfo ? Number(corpInfo.treasuryBalance).toFixed(4) : '0'}</div>
          <div className="text-sm text-gray-500">ETH</div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <div className="text-sm text-gray-400 mb-1">Total Received</div>
          <div className="text-3xl font-bold text-blue-300">{Number(totalReceived).toFixed(4)}</div>
          <div className="text-sm text-gray-500">ETH</div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <div className="text-sm text-gray-400 mb-1">Total Spent</div>
          <div className="text-3xl font-bold text-orange-300">{Number(totalSpent).toFixed(4)}</div>
          <div className="text-sm text-gray-500">ETH</div>
        </div>
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-gray-200">Treasury Info</h2>
        <p className="text-sm text-gray-400 mb-4">
          The treasury holds ETH and ERC20 tokens for the corporation. Spending requires governance approval through proposals.
        </p>
        <div className="bg-gray-800/50 rounded-lg p-4">
          <div className="text-xs text-gray-500 mb-1">Treasury Address</div>
          <div className="font-mono text-sm text-gray-300">{addresses.treasury || 'Not configured'}</div>
        </div>
        <div className="bg-gray-800/50 rounded-lg p-4 mt-3">
          <div className="text-xs text-gray-500 mb-1">How to fund</div>
          <div className="text-sm text-gray-400">
            Send ETH directly to the Corporation contract address or the Treasury address. ETH sent to the Corporation is automatically forwarded to the Treasury.
          </div>
        </div>
      </div>
    </div>
...
function DividendsPage({ loading, claimDividend, provider, addresses, account }: {
  loading: boolean
  claimDividend: (roundId: number) => Promise<void>
  provider: ethers.BrowserProvider | null
  addresses: typeof DEFAULT_ADDRESSES
  account: string
}) {
  ...
  const checkUnclaimed = async () => {
    ...
    try {
      ...
      console.error('Failed to check unclaimed:', err)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <div className="text-sm text-gray-400 mb-1">Distribution Rounds</div>
          <div className="text-3xl font-bold text-violet-300">{currentRound}</div>
          <div className="text-sm text-gray-500">total rounds</div>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          <div className="text-sm text-gray-400 mb-1">Distribution Method</div>
          <div className="text-xl font-bold text-indigo-300">Pull-based (Claim)</div>
          <div className="text-sm text-gray-500">Proportional to share ownership</div>
        </div>
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-gray-200">Check Unclaimed Dividends</h2>
        ...
        {unclaimedAmount !== '0' && (
          <div className="bg-emerald-900/20 border border-emerald-700/30 rounded-lg p-4">
            <div className="text-sm text-emerald-400">Unclaimed Amount: <strong>{unclaimedAmount} ETH</strong></div>
          </div>
        )}
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-gray-200">Claim Dividend</h2>
        <div className="flex gap-3">
          <input
            type="number"
            placeholder="Round ID"
          ...
          />
          <button
            onClick={() => claimDividend(Number(claimRoundId))}
            disabled={loading || !claimRoundId}
            className="bg-violet-600 hover:bg-violet-500 disabled:bg-gray-700 text-white px-6 py-2 rounded-lg text-sm font-medium transition-colors"
          >
...
function OfficersPage({ provider, addresses }: {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold">Officers & Directors</h1>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-gray-200">Corporate Officers</h2>
        <div className="space-y-3">
          <OfficerCard title="Chief Executive Officer (CEO)" address={formatAddr(ceo)} icon="👤" />
          <OfficerCard title="Chief Financial Officer (CFO)" address={formatAddr(cfo)} icon="💼" />
          <OfficerCard title="Secretary" address={formatAddr(secretary)} icon="📋" />
        </div>
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-4 text-gray-200">Board of Directors</h2>
        {directors.length === 0 ? (
          <div className="text-center py-8 text-gray-500">No directors appointed</div>
        ) : (
            ...
            {directors.map((dir, i) => (
              <div key={i} className="flex items-center justify-between bg-gray-800/50 rounded-lg p-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-mono text-sm text-gray-300">{dir}</span>
                </div>
                <span className="text-xs bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded">Director</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <h2 className="text-lg font-semibold mb-3 text-gray-200">How to Manage Officers</h2>
        <p className="text-sm text-gray-400">
          Officers and directors are appointed or removed through governance proposals. Create a proposal of type &quot;Officer Election&quot; in the Governance tab to appoint or remove officers.
        </p>
      </div>
    </div>
  )
...
function OfficerCard({ title, address, icon }: { title: string; address: string; icon: string }) {
  return (
